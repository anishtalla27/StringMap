...
with tempfile.TemporaryDirectory(prefix='stringmap-screenshots-') as temporary:
 ...
 assert len(files)==7,f'Expected 7 final screens, found {len(files)}'
 manifest_path=args.out/'manifest.json'
 manifest=json.loads(manifest_path.read_text()) if manifest_path.exists() else {}
 manifest[args.device]=dict(configuration='Release',resultBundle=str(args.result),screenshots=sorted(files,key=lambda x:x['file']))
